...
@section('primary_color', '#eab308')
@section('toggle_icon_color', 'text-yellow-500')
@section('glow_colors', '#eab308, #facc15, #eab308')
...
@section('content')
    <section class="w-full lg:w-[55%] min-h-screen p-10 lg:p-24 flex flex-col items-center justify-center bg-slate-50 dark:bg-[#0c0c0e] relative overflow-hidden text-center {{ app()->getLocale() == 'ar' ? 'order-1 lg:order-2' : 'order-1' }}">
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-yellow-500/10 rounded-full blur-[120px]"></div>
        <div class="relative z-10 w-full max-w-2xl mx-auto flex flex-col items-center">
            <div class="floating inline-block w-full max-w-sm mb-12">
                <div class="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 p-12 rounded-[2.5rem] shadow-2xl relative overflow-hidden min-h-[320px] flex flex-col items-center justify-center">
                    <span class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[12rem] font-[900] text-slate-300/70 dark:text-zinc-800/50 select-none z-0">429</span>
                    <div class="relative z-10 flex flex-col items-center w-full h-full">
                        <div class="w-24 h-24 bg-yellow-500/10 rounded-full mb-20 flex items-center justify-center backdrop-blur-sm">
            ...
            <div id="scroll-indicator" class="lg:hidden flex flex-col items-center gap-2 transition-all duration-500">
                <div class="w-[20px] h-[32px] border-2 border-slate-300 dark:border-zinc-700 rounded-full relative">
                    <div class="w-1 h-2 bg-yellow-500 rounded-full absolute top-1.5 left-1/2 -translate-x-1/2 animate-scroll-ar"></div>
                </div>
    ...
    <section class="w-full lg:w-[45%] min-h-screen flex flex-col justify-center px-8 py-12 lg:px-16 relative z-10 bg-white dark:bg-zinc-950 border-slate-100 dark:border-zinc-900 {{ app()->getLocale() == 'ar' ? 'border-r lg:border-l order-2 lg:order-1 text-right' : 'border-l lg:border-r order-2 text-left' }}">
        <div class="w-full max-w-md mx-auto fade-in">
            <header class="mb-10">
                ...
                <div class="inline-flex items-center justify-center gap-2 mb-6 px-4 py-2 rounded-full bg-yellow-500/10 text-yellow-600 font-bold text-sm">
                    <span>{{ __('errors.429.title') }}</span>
                </div>
                <h1 class="text-3xl lg:text-4xl font-[900] text-slate-900 dark:text-white mb-4 leading-relaxed py-2 glow-text">{{ __('errors.429.title') }}</h1>
                <p class="text-slate-500 dark:text-zinc-400 font-medium text-lg leading-relaxed">{{ __('errors.429.desc') }}</p>
            </header>
            <div class="flex flex-col sm:flex-row gap-4 mt-8">
                <button onclick="window.location.href = window.location.href" class="flex-1 flex items-center justify-center bg-yellow-500 text-white py-3 px-5 rounded-2xl font-black text-base shadow-[0_10px_30px_rgba(234,179,8,0.3)] hover:scale-[1.02] active:scale-[0.98] transition-all duration-300">{{ __('errors.go_back') }}</button>
                <a href="{{ url('/') }}" class="flex-1 flex items-center justify-center bg-slate-100 dark:bg-zinc-900 text-slate-700 dark:text-zinc-300 border border-slate-200 dark:border-zinc-800 py-3 px-5 rounded-2xl font-bold text-base hover:bg-slate-200 dark:hover:bg-zinc-800 transition-all duration-300">{{ __('errors.back_home') }}</a>
            </div>
        </div>
    </section>
@endsection