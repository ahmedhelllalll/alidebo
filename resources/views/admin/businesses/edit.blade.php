        ...
        <div class="relative bg-white dark:bg-zinc-900/40 border border-slate-200/60 dark:border-white/5  shadow-sm reveal-item group/card hover:shadow-md transition-shadow duration-500">
            ...
            <div class="p-8 pt-10">
                ...
                <div class="space-y-6 lg:col-span-2">
                    <div class="flex items-center justify-between border-b border-slate-100 dark:border-white/5 pb-5">
                        ...
                        <div class="flex items-center gap-3">
                            <button type="button" onclick="promptCreateCategoryAdmin()" class="flex items-center gap-2 px-3 py-1.5 bg-primary text-white rounded-lg text-xs font-bold hover:bg-primary/90 transition-all active:scale-[0.98] shadow-sm whitespace-nowrap">
                                <i aria-hidden="true" class="fa-solid fa-plus"></i> New Category
                            </button>
                    ...
                    <div id="adminCategoriesWrapper" class="space-y-6">
                        {{-- GENERAL CATEGORY --}}
                        @if($generalMedia->count() > 0 || $categories->count() == 0)
                        <div class="admin-category-block bg-slate-50/50 dark:bg-white/[0.02] p-5 rounded-2xl border border-slate-200 dark:border-white/10" data-category-id="">
                            <div class="flex justify-between items-center mb-4">
                                <h4 class="text-xs font-black text-slate-700 dark:text-zinc-200 uppercase tracking-widest">General Gallery <span class="text-slate-400 font-bold ms-2">(<span class="cat-count">{{ $generalMedia->count() }}</span>/10)</span></h4>
                            </div>
                            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-6 admin-gallery-grid-container">
                                <div class="relative aspect-square border-2 border-dashed border-slate-200 dark:border-white/10 flex items-center justify-center hover:border-purple-400/40 transition-all duration-500 cursor-pointer group/add bg-gradient-to-br from-slate-50/50 to-slate-100/20 dark:from-white/[0.02] dark:to-white/[0.01] hover:from-purple-50/30 hover:to-purple-100/10">
                                    ...
                                    <input type="file" name="gallery[]" multiple accept="image/*" onchange="handleAdminCategoryUpload(this, '')" class="absolute inset-0 opacity-0 cursor-pointer z-10">
                                </div>
                                @foreach($generalMedia as $media)
                                    <div class="relative aspect-square rounded-2xl overflow-hidden group/item shadow-sm border border-slate-200/80 dark:border-white/10 admin-gallery-item" data-id="{{ $media->id }}">
                                        <img src="{{ $media->file_url }}" class="w-full h-full object-cover">
                                        <div class="absolute inset-0 bg-slate-900/40 opacity-0 group-hover/item:opacity-100 transition-opacity flex items-center justify-center">
                                            <button type="button" onclick="deleteAdminMedia({{ $media->id }}, this)" class="w-8 h-8 rounded-full bg-red-500 text-white flex items-center justify-center hover:bg-red-600 shadow-lg scale-90 group-hover/item:scale-100 transition-all">
                        ...
                        {{-- CUSTOM CATEGORIES --}}
                        @foreach($categories as $category)
                        <div class="admin-category-block bg-slate-50/50 dark:bg-white/[0.02] p-5 rounded-2xl border border-slate-200 dark:border-white/10" data-category-id="{{ $category->id }}">
                            <div class="flex justify-between items-center mb-4">
                                <div class="flex items-center gap-3">
                                    <h4 class="text-[13px] font-black text-slate-800 dark:text-white uppercase tracking-widest">{{ $category->name }} <span class="text-xs font-medium text-slate-400 ms-2">(<span class="cat-count">{{ $category->media->count() }}</span>/10)</span></h4>
                                    <button type="button" onclick="promptEditCategoryAdmin({{ $category->id }}, '{{ addslashes($category->name) }}')" class="w-6 h-6 rounded-lg bg-blue-500/10 text-blue-500 flex items-center justify-center hover:bg-blue-500 hover:text-white transition-colors" title="Edit Category"><i class="fa-solid fa-pen text-[10px]"></i></button>
                                    <button type="button" onclick="deleteAdminCategory({{ $category->id }}, this)" class="w-6 h-6 rounded-lg bg-red-500/10 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-colors" title="Delete Category"><i class="fa-solid fa-trash text-[10px]"></i></button>
                                </div>
                            </div>
                            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-6 admin-gallery-grid-container">
                                <div class="relative aspect-square border-2 border-dashed border-slate-200 dark:border-white/10 flex items-center justify-center hover:border-purple-400/40 transition-all duration-500 cursor-pointer group/add bg-gradient-to-br from-slate-50/50 to-slate-100/20 dark:from-white/[0.02] dark:to-white/[0.01] hover:from-purple-50/30 hover:to-purple-100/10">
                                    ...
                                    <input type="file" name="gallery[]" multiple accept="image/*" onchange="handleAdminCategoryUpload(this, '{{ $category->id }}')" class="absolute inset-0 opacity-0 cursor-pointer z-10">
                                </div>
...
<script>
    ...
    window.showCustomModal = function(options) {
        const { type = 'confirm', title, desc = '', placeholder = '', defaultValue = '', confirmText = '{{ __("admin.save") ?? "Save" }}', cancelText = '{{ __("admin.cancel") ?? "Cancel" }}', confirmClass = 'bg-purple-500 hover:bg-purple-600 shadow-purple-500/30', onConfirm } = options;
        ...
        let inputHtml = '';
        if (type === 'input') {
            inputHtml = `<input type="text" id="customModalInput" value="${defaultValue}" placeholder="${placeholder}" class="w-full mt-4 bg-transparent border border-slate-300 dark:border-white/20 rounded-xl px-4 py-2.5 text-sm font-medium text-slate-800 dark:text-white focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all">`;
        }
    ...
    window.promptCreateCategoryAdmin = function() {
        showCustomModal({
            ...
            onConfirm: (name) => {
                if(!name) return;
                return fetch('{{ route("admin.businesses.categories.store", $business->id) }}', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' },
                    body: JSON.stringify({ name })
                }).then(r => r.json()).then(d => {
                    if(d.success) {
                        if(typeof showToast !== 'undefined') showToast('success', 'Category Created');
                        ...
                        if(wrapper) {
                            ...
                            <div class="flex justify-between items-center mb-4">
                                <div class="flex items-center gap-3">
                                    <h4 class="text-[13px] font-black text-slate-800 dark:text-white uppercase tracking-widest">${d.category.name} <span class="text-xs font-medium text-slate-400 ms-2">(<span class="cat-count">0</span>/10)</span></h4>
                                    <button type="button" onclick="promptEditCategoryAdmin(${d.category.id}, '${escapedName}')" class="w-6 h-6 rounded-lg bg-blue-500/10 text-blue-500 flex items-center justify-center hover:bg-blue-500 hover:text-white transition-colors" title="Edit Category"><i class="fa-solid fa-pen text-[10px]"></i></button>
                                    <button type="button" onclick="deleteAdminCategory(${d.category.id}, this)" class="w-6 h-6 rounded-lg bg-red-500/10 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-colors" title="Delete Category"><i class="fa-solid fa-trash text-[10px]"></i></button>
                                </div>
                            </div>
                            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-6 admin-gallery-grid-container">
                                <div class="relative aspect-square border-2 border-dashed border-slate-200 dark:border-white/10 flex items-center justify-center hover:border-purple-400/40 transition-all duration-500 cursor-pointer group/add bg-gradient-to-br from-slate-50/50 to-slate-100/20 dark:from-white/[0.02] dark:to-white/[0.01] hover:from-purple-50/30 hover:to-purple-100/10">
                                    ...
                                    <input type="file" name="gallery[]" multiple accept="image/*" onchange="handleAdminCategoryUpload(this, '${d.category.id}')" class="absolute inset-0 opacity-0 cursor-pointer z-10">
                                </div>
                            </div>`;
                        ...
                        if(typeof showToast !== 'undefined') showToast('error', d.message || 'Could not create category.');
                    }
                }).catch(() => {
                    if(typeof showToast !== 'undefined') showToast('error', 'Connection problem.');
                });
            }
        });
    };

    window.promptEditCategoryAdmin = function(id, currentName) {
        showCustomModal({
            type: 'input',
            title: '{{ __("admin.edit") ?? "Edit Category" }}',
            defaultValue: currentName,
            placeholder: '{{ __("admin.category_name") ?? "Category Name" }}',
            confirmClass: 'bg-blue-500 hover:bg-blue-600 shadow-blue-500/30',
            onConfirm: (name) => {
                if(!name || name === currentName) return;
                return fetch(`{{ url('admin/businesses/'.$business->id.'/categories') }}/${id}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' },
                    body: JSON.stringify({ name })
                }).then(r => r.json()).then(d => {
                    if(d.success) {
                        if(typeof showToast !== 'undefined') showToast('success', 'Category Updated');
                        const block = document.querySelector(`.admin-category-block[data-category-id="${id}"]`);
                        if(block) {
                            ...
                            if(editBtn) {
                                const escapedName = d.category.name.replace(/'/g, "\\'");
                                editBtn.setAttribute('onclick', `promptEditCategoryAdmin(${id}, '${escapedName}')`);
                            }
                        }
                    } else {
                        if(typeof showToast !== 'undefined') showToast('error', d.message || 'Could not update category.');
                    }
                }).catch(() => {
                    if(typeof showToast !== 'undefined') showToast('error', 'Connection problem.');
                });
    ...
    window.deleteAdminCategory = function(id, btn) {
        showCustomModal({
            type: 'confirm',
            title: '{{ __("admin.confirm_delete") ?? "Delete Category?" }}',
            desc: '{{ __("admin.delete_category_desc") ?? "Are you sure you want to delete this category and all its pending images?" }}',
            confirmText: '{{ __("admin.delete") ?? "Delete" }}',
            ...
            onConfirm: () => {
                return fetch(`{{ url('admin/businesses/'.$business->id.'/categories') }}/${id}`, {
                    ...
                    if(d.success) {
                        btn.closest('.admin-category-block').remove();
                        if(typeof showToast !== 'undefined') showToast('success', 'Category Deleted');
                        updateAdminGalleryCount();
                    } else {
                        if(typeof showToast !== 'undefined') showToast('error', d.message || 'Could not delete category.');
                    }
                }).catch(() => {
                    if(typeof showToast !== 'undefined') showToast('error', 'Connection problem.');
                });
    ...
    window.handleAdminCategoryUpload = function(input, categoryId) {
        if (!input.files || !input.files.length) return;
        const block = input.closest('.admin-category-block');
        const grid = block.querySelector('.admin-gallery-grid-container');
        const currentCount = grid.querySelectorAll('.admin-gallery-item').length;
        if(currentCount + input.files.length > 10) {
            if(typeof showToast !== 'undefined') showToast('warning', `Limit reached. You can only have 10 images per category.`);
            input.value = ''; return;
        }
        ...
        fetch('{{ route("admin.businesses.media.upload", $business->id) }}', {
            method: 'POST',
            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' },
            body: formData
            ...
            input.value = '';
            if(data.success) {
                if(typeof showToast !== 'undefined') showToast('success', 'Images uploaded');
                data.media.forEach(m => {
                    const addButton = block.querySelector('.group\\/add');
                    const div = document.createElement('div');
                    div.className = 'relative aspect-square rounded-2xl overflow-hidden group/item shadow-sm border border-slate-200/80 dark:border-white/10 admin-gallery-item opacity-0 scale-90';
                    div.dataset.id = m.id;
                    div.innerHTML = `
                        <img src="${m.file_url}" class="w-full h-full object-cover">
                        <div class="absolute inset-0 bg-slate-900/40 opacity-0 group-hover/item:opacity-100 transition-opacity flex items-center justify-center">
                            <button type="button" onclick="deleteAdminMedia(${m.id}, this)" class="w-8 h-8 rounded-full bg-red-500 text-white flex items-center justify-center hover:bg-red-600 shadow-lg scale-90 group-hover/item:scale-100 transition-all">
                                <i class="fa-solid fa-trash text-[10px]"></i>
                            </button>
                        </div>
                    `;
                    grid.insertBefore(div, addButton.nextSibling); // Add after the plus button
                ...
                countSpan.textContent = grid.querySelectorAll('.admin-gallery-item').length;
                updateAdminGalleryCount();
            } else {
                countSpan.textContent = currentCount;
                if(typeof showToast !== 'undefined') showToast('error', data.error || 'Upload failed');
            }
        }).catch(() => {
            countSpan.textContent = currentCount;
            if(typeof showToast !== 'undefined') showToast('error', 'Connection error');
        });
    };

    window.deleteAdminMedia = function(id, btn) {
        if(!confirm('Delete this image?')) return;
        fetch(`{{ url('admin/businesses/'.$business->id.'/media') }}/${id}`, {
            method: 'DELETE',
            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' }
        }).then(r => r.json()).then(d => {
            if(d.success) {
                const block = btn.closest('.admin-category-block');
                const el = btn.closest('.admin-gallery-item');
                if(typeof gsap !== 'undefined') {
                    gsap.to(el, { opacity: 0, scale: 0.8, duration: 0.3, onComplete: () => { 
                        el.remove(); 
                        if(block) block.querySelector('.cat-count').textContent = block.querySelectorAll('.admin-gallery-item').length;
                        updateAdminGalleryCount(); 
                    } });
                } else {
                    el.remove();
                ...
                if(typeof showToast !== 'undefined') showToast('success', 'Image deleted');
            } else {
                if(typeof showToast !== 'undefined') showToast('error', 'Could not delete image.');
            }
        }).catch(() => {
            if(typeof showToast !== 'undefined') showToast('error', 'Connection error.');
        });
    ...
    window.updateAdminGalleryCount = function() {
        const count = document.querySelectorAll('.admin-gallery-item').length || 0;
