    ...
    <x-admin.table>
        <x-slot name="header">
            <tr class="bg-slate-50/50 dark:bg-zinc-800/20 border-b border-slate-100 dark:border-zinc-800/60">
                ...
                <th scope="col" class="px-6 py-5 text-start text-[12px] font-[900] text-slate-500 dark:text-zinc-400 uppercase tracking-widest">{{ __('admin.indexing') }}</th>
        ...
        <tbody id="business-table-body" class="divide-y divide-slate-50 dark:divide-white/[0.03]">
            ...
            <tr class="hover:bg-slate-50/80 dark:hover:bg-zinc-800/30 transition-all group business-row" data-id="{{ $business->id }}" style="--row-index: {{ $index }}">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                    @php
                        $log = \App\Models\GoogleIndexLog::where('indexable_type', 'App\Models\BusinessProfile')->where('indexable_id', $business->id)->first();
                    @endphp
                    @if($log)
                    ...
                    @else
                        <form action="{{ route('admin.indexing.request') }}" method="POST" class="inline">
                            @csrf
                            <input type="hidden" name="url" value="{{ route('business.view', $business->slug) }}">
                            <input type="hidden" name="indexable_type" value="App\Models\BusinessProfile">
                            <input type="hidden" name="indexable_id" value="{{ $business->id }}">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-end">
                    <div class="flex items-center justify-end gap-2">
                        @if(!$business->owner_id)
                        <button onclick="copyClaimLink({{ $business->id }})" class="p-2 text-slate-400 hover:text-emerald-500 hover:bg-emerald-500/10 rounded-xl transition-all tooltip" title="{{ __('admin.generate_claim_link') ?? 'Generate Claim Link' }}">
...
<div class="md:hidden divide-y divide-slate-100 dark:divide-zinc-800/60">
    ...
    <div class="mobile-card p-5 hover:bg-slate-50/50 dark:hover:bg-zinc-800/20 transition-all business-row" data-id="{{ $business->id }}" style="--row-index: {{ $index }}">
        ...
        <div class="mt-4 pt-4 border-t border-slate-50 dark:border-white/[0.03]">
            @php
                $logMobile = \App\Models\GoogleIndexLog::where('indexable_type', 'App\Models\BusinessProfile')->where('indexable_id', $business->id)->first();
            @endphp
            @if($logMobile)
