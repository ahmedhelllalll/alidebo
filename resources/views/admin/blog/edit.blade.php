...
@section('title', __('admin.update_blog'))

@section('content')
<div class="space-y-6 lg:space-y-8">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 dashboard-header-reveal">
        <div>
            <h1 class="text-2xl sm:text-3xl font-[900] tracking-tight bg-gradient-to-r from-slate-900 to-slate-600 dark:from-white dark:to-zinc-400 bg-clip-text text-transparent">{{ __('admin.update_blog') }}: {{ $blog->title['en'] ?? '' }}</h1>
        </div>
    ...
    @php
        $initialMediaType = old('media_type', $blog->media_type ?? 'none');
        if ($initialMediaType === 'video') {
            if ($blog->media_url && !\Illuminate\Support\Str::startsWith($blog->media_url, 'http')) {
                $initialMediaType = 'video_upload';
            } else {
                $initialMediaType = 'video_embed';
            }
        }
    @endphp
    ...
    <form action="{{ route('admin.blogs.update', $blog) }}" method="POST" enctype="multipart/form-data" class="dashboard-card-reveal" novalidate
        ...
        <div class="bg-white dark:bg-[#121214] shadow-[0_10px_40px_rgba(0,0,0,0.04)] dark:shadow-[0_10px_40px_rgba(0,0,0,0.4)] border border-slate-200 dark:border-white/10 rounded-2xl p-6 space-y-6">
                ...
                <div>
                    ...
                    @foreach(['en', 'ar', 'es', 'de', 'zh', 'tr'] as $loc)
                        <div x-show="activeTab === '{{ $loc }}'" x-cloak>
                            <input type="text" name="title[{{ $loc }}]" value="{{ old('title.'.$loc, $blog->title[$loc] ?? '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white" placeholder="{{ strtoupper($loc) }} Title" {{ $loc === 'en' ? 'required' : '' }}>
                        </div>
                    @endforeach
                </div>
                <div>
            ...
            <div>
                ...
                @foreach(['en', 'ar', 'es', 'de', 'zh', 'tr'] as $loc)
                    <div x-show="activeTab === '{{ $loc }}'" x-cloak class="mt-1">
                        <textarea name="content[{{ $loc }}]" rows="15" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white font-mono p-4" placeholder="{{ strtoupper($loc) }} Markdown Content...">{{ old('content.'.$loc, $blog->content[$loc] ?? '') }}</textarea>
                    </div>
                @endforeach
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                @php
                    $currentStatus = old('status', $blog->status);
                ...
                <div class="input-group col-span-1 md:col-span-2 space-y-4">
                    ...
                    <div x-show="mediaType === 'image'" x-cloak class="mt-4">
                        ...
                        @if($blog->media_type === 'image' && $blog->media_url)
                            <div class="mt-4" x-show="!imagePreview">
                                <p class="text-xs text-slate-400 mb-2">{{ __('admin.current_image') }}:</p>
                                <img src="{{ asset('storage/' . $blog->media_url) }}" alt="{{ __('admin.current_image') }}" class="w-48 h-32 object-cover rounded-lg border border-slate-200 shadow-sm">
                            </div>
                        @endif
                    ...
                    <div x-show="mediaType === 'video_embed'" x-cloak class="mt-4 p-4 rounded-2xl bg-slate-50 dark:bg-zinc-800/50 border border-slate-200 dark:border-white/5">
                        ...
                        <input type="url" name="video_url" id="video_url" value="{{ old('video_url', ($blog->media_type === 'video' && \Illuminate\Support\Str::startsWith($blog->media_url, 'http')) ? $blog->media_url : '') }}" placeholder="https://www.youtube.com/embed/..." class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white">
                        
                        @if($blog->media_type === 'video' && $blog->media_url && \Illuminate\Support\Str::startsWith($blog->media_url, 'http'))
                            <div class="mt-4 aspect-video w-full max-w-sm rounded-lg overflow-hidden border border-slate-200">
                                <iframe src="{{ $blog->media_url }}" class="w-full h-full" frameborder="0" allowfullscreen></iframe>
                            </div>
                        @endif
                    </div>
                    ...
                    <div x-show="mediaType === 'video_upload'" x-cloak class="mt-4">
                        ...
                        @if($blog->media_type === 'video' && $blog->media_url && !\Illuminate\Support\Str::startsWith($blog->media_url, 'http'))
                            <div class="mt-4 aspect-video w-full max-w-sm rounded-lg overflow-hidden border border-slate-200 shadow-sm" x-show="!videoPreview">
                                <video src="{{ asset('storage/' . $blog->media_url) }}" class="w-full h-full object-cover" controls muted></video>
                            </div>
                        @endif
                    ...
                    <div x-show="mediaType !== 'none'" x-cloak class="mt-6 border-t border-slate-200 dark:border-white/10 pt-6">
                        ...
                        @foreach(['en', 'ar', 'es', 'de', 'zh', 'tr'] as $loc)
                            <div x-show="activeTab === '{{ $loc }}'" x-cloak>
                                <input type="text" name="media_alt[{{ $loc }}]" value="{{ old('media_alt.'.$loc, $blog->media_alt[$loc] ?? '') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white" placeholder="{{ strtoupper($loc) }} Alt Text">
                            </div>
                        @endforeach
