...
<div class="space-y-6 lg:space-y-8">
    ...
    <form action="{{ route('admin.blogs.store') }}" method="POST" enctype="multipart/form-data" class="dashboard-card-reveal" novalidate
          x-data="{ 
              activeTab: 'en', 
              mediaType: 'image', 
              imagePreview: null, 
        ...
        <div class="bg-white dark:bg-[#121214] shadow-[0_10px_40px_rgba(0,0,0,0.04)] dark:shadow-[0_10px_40px_rgba(0,0,0,0.4)] border border-slate-200 dark:border-white/10 rounded-2xl p-6 space-y-6">
            ...
            <div>
                ...
                @foreach(['en', 'ar', 'es', 'de', 'zh', 'tr'] as $loc)
                    <div x-show="activeTab === '{{ $loc }}'" x-cloak class="mt-1">
                        <textarea name="description[{{ $loc }}]" rows="3" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white" placeholder="{{ strtoupper($loc) }} Description"></textarea>
                    </div>
                @endforeach
            </div>
            ...
            <div>
                ...
                @foreach(['en', 'ar', 'es', 'de', 'zh', 'tr'] as $loc)
                    <div x-show="activeTab === '{{ $loc }}'" x-cloak class="mt-1">
                        <textarea name="content[{{ $loc }}]" rows="15" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white font-mono" placeholder="{{ strtoupper($loc) }} Content (Markdown supported)"></textarea>
                    </div>
                @endforeach
            </div>
