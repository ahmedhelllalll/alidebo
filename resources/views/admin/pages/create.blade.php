...
<div class="space-y-6 lg:space-y-8">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 dashboard-header-reveal">
        <div>
            <h1 class="text-2xl sm:text-3xl font-[900] tracking-tight bg-gradient-to-r from-slate-900 to-slate-600 dark:from-white dark:to-zinc-400 bg-clip-text text-transparent">{{ __('admin.add_page') }}</h1>
        </div>
    ...
    <form action="{{ route('admin.pages.store') }}" method="POST" enctype="multipart/form-data" class="dashboard-card-reveal">
        @csrf
        <div class="bg-white dark:bg-[#121214] shadow-[0_10px_40px_rgba(0,0,0,0.04)] dark:shadow-[0_10px_40px_rgba(0,0,0,0.4)] border border-slate-200 dark:border-white/10 rounded-2xl p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="title[en]" id="title_en" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white" required>
                </div>
                <div>
            ...
            <div>
                ...
                <textarea name="content[en]" id="content_en" rows="12" class="tinymce-editor mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-800 dark:border-gray-600 dark:text-white"></textarea>
            </div>
                ...
                <div class="input-group">
                    ...
                    <div class="relative custom-smart-dropdown" id="dropdown_location">
                        <div class="relative flex items-center group">
                            ...
                            <input type="hidden" name="location" id="field_location" value="hidden" required aria-required="true" class="validate-target">
                            <button type="button" class="dropdown-trigger w-full input-premium rounded-2xl py-5 ps-14 pe-6 text-sm font-bold flex items-center justify-between transition-all duration-300 outline-none cursor-pointer text-slate-800 dark:text-white">
                                <span class="dropdown-label truncate">{{ __('admin.hidden') }}</span>
                                <i aria-hidden="true" class="fa-solid fa-chevron-down text-[10px] text-slate-300 dark:text-zinc-600 transition-transform duration-300"></i>
                            </button>
                        </div>
                        <div class="dropdown-menu hidden absolute z-50 mt-2 w-full bg-white dark:bg-[#121214] border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden opacity-0 translate-y-2">
                ...
                <div class="input-group">
                    ...
                    <div class="relative custom-smart-dropdown" id="dropdown_layout">
                        <div class="relative flex items-center group">
                            ...
                            <input type="hidden" name="layout_style" id="field_layout" value="default" required aria-required="true" class="validate-target">
                            <button type="button" class="dropdown-trigger w-full input-premium rounded-2xl py-5 ps-14 pe-6 text-sm font-bold flex items-center justify-between transition-all duration-300 outline-none cursor-pointer text-slate-800 dark:text-white">
                                <span class="dropdown-label truncate">{{ __('admin.default') }}</span>
                                <i aria-hidden="true" class="fa-solid fa-chevron-down text-[10px] text-slate-300 dark:text-zinc-600 transition-transform duration-300"></i>
                            </button>
                        </div>
                        <div class="dropdown-menu hidden absolute z-50 mt-2 w-full bg-white dark:bg-[#121214] border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden opacity-0 translate-y-2">
