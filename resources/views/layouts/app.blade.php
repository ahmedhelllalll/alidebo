...
<body class="rtl:font-cairo ltr:font-jakarta bg-white dark:bg-[#09090b] text-slate-900 dark:text-white transition-colors duration-300 overflow-x-hidden antialiased">
    ...
    <!-- PWA Install Prompt -->
    <div id="pwa-install-prompt" class="fixed bottom-4 left-4 right-4 md:hidden z-[99999] bg-white dark:bg-[#0e0e11] border border-gray-200 dark:border-gray-800 rounded-2xl shadow-2xl p-5 transform transition-all duration-500 translate-y-[150%] opacity-0 pointer-events-none flex flex-col gap-3">
    ...
    <script>
        ...
        window.addEventListener('beforeinstallprompt', (e) => {
            ...
            if (!localStorage.getItem('pwa_prompt_dismissed')) {
                ...
                setTimeout(() => {
                    pwaPrompt.classList.remove('translate-y-[150%]', 'opacity-0', 'pointer-events-none');
                }, 1500);
            }
        });
        ...
        if (pwaInstallBtn) {
            pwaInstallBtn.addEventListener('click', async () => {
                if (deferredPrompt) {
                    pwaPrompt.classList.add('translate-y-[150%]', 'opacity-0', 'pointer-events-none');
