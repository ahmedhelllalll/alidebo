...
@section('title', __('forms.register_title'))
...
<div class="text-center w-full space-y-3">
    <h1 class="text-4xl lg:text-5xl font-black text-slate-900 dark:text-white tracking-tighter">{{ __('forms.register_header') }}</h1>
    <p class="text-slate-400 dark:text-zinc-500 font-black uppercase tracking-[0.2em] text-[10px]">{{ __('forms.register_desc') }}</p>
</div>
@endsection
...
<form id="register-form" method="POST" action="{{ route('register') }}" class="space-y-4">
    ...
    <div class="space-y-4 text-start">
        <label class="text-[10px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-[0.25em] px-2 block">{{ __('forms.full_name') }}</label>
        <input id="name" type="text" name="name" value="{{ old('name') }}" required autofocus placeholder="{{ __('forms.name_placeholder') }}"
        ...
        @if($errors->has('name'))
            <p class="mt-2 font-black text-red-500 text-[10px] uppercase tracking-tighter px-2">{{ __('forms.invalid_name') }}</p>
        @endif
    </div>

    <div class="space-y-4 text-start">
        <label class="text-[10px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-[0.25em] px-2 block">{{ __('forms.email') }}</label>
        <input id="email" type="email" name="email" value="{{ old('email') }}" required placeholder="{{ __('forms.email_placeholder') }}" dir="ltr"
            class="w-full px-6 py-5 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl focus:ring-8 focus:ring-primary/5 focus:border-primary outline-none transition-all dark:text-white placeholder:text-slate-200 dark:placeholder:text-zinc-800 font-black text-sm shadow-sm" />
        @if($errors->has('email'))
            <p class="mt-2 font-black text-red-500 text-[10px] uppercase tracking-tighter px-2">{{ __('forms.invalid_email') }}</p>
        @endif
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="space-y-4 text-start">
            <label class="text-[10px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-[0.25em] px-2 block">{{ __('forms.password') }}</label>
            <input id="password" type="password" name="password" required placeholder="{{ __('forms.password_placeholder') }}" dir="ltr"
                class="w-full px-6 py-5 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl focus:ring-8 focus:ring-primary/5 focus:border-primary outline-none transition-all dark:text-white placeholder:text-slate-200 dark:placeholder:text-zinc-800 font-black text-sm shadow-sm" />
            @if($errors->has('password'))
                <p class="mt-2 font-black text-red-500 text-[10px] uppercase tracking-tighter px-2">{{ __('forms.password_min_length') }}</p>
            @endif
        </div>

        <div class="space-y-4 text-start">
            <label class="text-[10px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-[0.25em] px-2 block">{{ __('forms.confirm_password') }}</label>
            <input id="password_confirmation" type="password" name="password_confirmation" required placeholder="{{ __('forms.password_placeholder') }}" dir="ltr"
                class="w-full px-6 py-5 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl focus:ring-8 focus:ring-primary/5 focus:border-primary outline-none transition-all dark:text-white placeholder:text-slate-200 dark:placeholder:text-zinc-800 font-black text-sm shadow-sm" />
        </div>
    </div>

    <div class="flex items-center gap-3 px-1 justify-start rtl:justify-start ltr:justify-end group cursor-pointer" onclick="document.getElementById('show_passwords').click()">
        <input id="show_passwords" type="checkbox" onclick="event.stopPropagation(); toggleAllPasswords()" class="custom-checkbox">
        <label class="text-sm font-bold text-slate-500 dark:text-zinc-400 cursor-pointer select-none group-hover:text-primary transition-colors">{{ __('forms.show_passwords') }}</label>
    </div>
    ...
    <button type="submit" id="submit-btn" class="w-full flex items-center justify-center bg-primary text-white py-4 rounded-2xl font-black text-lg shadow-[0_10px_30px_rgba(244,80,24,0.3)] hover:scale-[1.01] active:scale-[0.98] transition-all duration-300 mt-2">
        <span id="btn-text">{{ __('forms.register_button') }}</span>
    ...
    <div class="relative py-3">
        ...
        <div class="relative flex justify-center text-xs uppercase">
            <span class="bg-white dark:bg-zinc-950 px-2 text-slate-500 font-bold">{{ __('forms.or_register_with') }}</span>
        </div>
    </div>
    ...
    <p class="text-center text-slate-500 dark:text-zinc-500 font-bold text-sm pt-4">
        {{ __('forms.have_account') }}
        <a href="{{ route('login') }}" class="text-primary hover:underline underline-offset-4">{{ __('forms.login') }}</a>
    </p>
</form>
@endsection
...
<div class="min-h-[100vh] lg:min-h-0 flex flex-col items-center justify-center w-full py-8 relative">
    <div class="fade-in flex flex-col items-center text-center px-6" style="animation-delay: 0.3s">
        ...
        <h2 class="text-4xl lg:text-6xl font-semibold tracking-tight text-slate-800 dark:text-zinc-100 leading-tight mb-4">
            {{ __('forms.register_visual_1') }}
            <span class="block mt-4 pb-4 font-[900] glow-text drop-shadow-[0_0_15px_rgba(244,80,24,0.3)]">{{ __('forms.register_visual_2') }}</span>
        </h2>
        <p class="text-base lg:text-lg font-medium text-slate-400 dark:text-zinc-500 max-w-md mb-8 px-4">
            {{ __('forms.register_visual_desc') }}
        </p>
    </div>
