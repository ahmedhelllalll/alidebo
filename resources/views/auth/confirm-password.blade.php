...
@section('title', __('forms.confirm_identity_title'))
...
<div class="text-center lg:text-start">
    <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-400 text-[10px] font-black mb-4 uppercase tracking-widest border border-amber-500/20">
        ...
        {{ __('forms.active_verification') }}
    </div>

    <h1 class="text-3xl font-[900] text-slate-900 dark:text-white mb-4">{{ __('forms.confirm_identity_header') }}</h1>
    ...
    <p class="text-slate-500 dark:text-zinc-400 font-semibold text-base leading-relaxed">
        {{ __('forms.confirm_identity_desc') }}
    </p>
</div>
@endsection
...
<form method="POST" action="{{ route('password.confirm') }}" id="confirm-form" class="space-y-6">
    ...
    <div class="space-y-2 text-start">
        <label for="password" class="block text-sm font-black text-slate-700 dark:text-zinc-300 me-2">{{ __('forms.password') }}</label>
        <input id="password" type="password" name="password" required autocomplete="current-password" dir="ltr" placeholder="{{ __('forms.password_placeholder') }}"
            class="block w-full px-5 py-4 bg-slate-50 dark:bg-zinc-900 border-2 border-slate-100 dark:border-zinc-800 rounded-2xl text-slate-900 dark:text-white font-bold outline-none focus:border-primary transition-all text-start" />

        @if($errors->has('password'))
            <p class="mt-2 me-2 font-bold text-xs text-red-500 text-start">
                {{ __('forms.incorrect_password') }}
            </p>
        @endif
    </div>
    ...
    <div class="flex items-center gap-3 px-1 justify-start group cursor-pointer" onclick="document.getElementById('show_passwords').click()">
        ...
        <label class="text-sm font-bold text-slate-500 dark:text-zinc-400 cursor-pointer select-none group-hover:text-primary transition-colors">{{ __('forms.show_password') }}</label>
    </div>
    ...
    <div class="pt-2">
        <button type="submit" id="submit-btn" class="w-full flex items-center justify-center bg-primary text-white py-4 rounded-2xl font-black text-lg shadow-[0_10px_30px_rgba(244,80,24,0.3)] hover:scale-[1.02] active:scale-[0.98] transition-all duration-300">
            <span id="btn-text">{{ __('forms.confirm_identity_btn') }}</span>
...
<div class="min-h-[100vh] lg:min-h-0 flex flex-col items-center justify-center w-full py-8 relative text-center">
    <div class="fade-in">
        ...
        <h2 class="text-4xl lg:text-5xl font-semibold tracking-tight text-slate-800 dark:text-zinc-100 leading-tight mb-6">
            {{ __('forms.your_privacy_is') }} 
            <span class="block mt-4 pb-4 font-[900] glow-text">{{ __('forms.first_digital_security') }}</span>
        </h2>

        <p class="text-lg font-medium text-slate-400 dark:text-zinc-500 max-w-sm mx-auto">
            {{ __('forms.extra_confirmation_desc') }}
        </p>
    </div>
</div>
@endsection
