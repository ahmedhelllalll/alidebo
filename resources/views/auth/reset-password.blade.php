...
@section('title', __('forms.reset_title'))
...
<div class="text-center space-y-3">
    <h1 class="text-4xl lg:text-5xl font-black text-slate-900 dark:text-white tracking-tighter">{{ __('forms.one_step_left') }}</h1>
...
<form method="POST" action="{{ route('password.store') }}" id="reset-form" class="space-y-5">
    ...
    <div class="space-y-4 text-start">
        <label class="block text-[10px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-[0.25em] px-2">{{ __('forms.email') }}</label>
        <input id="email" type="email" name="email" value="{{ old('email', $request->email) }}" required readonly dir="ltr" 
            class="block w-full px-6 py-5 bg-slate-50 dark:bg-zinc-900/30 border border-slate-200 dark:border-zinc-800 rounded-2xl text-slate-400 dark:text-zinc-600 font-black text-sm outline-none cursor-not-allowed text-start shadow-inner" />
    </div>

    {{-- كلمة المرور الجديدة --}}
    <div class="space-y-4 text-start">
        <label for="password" class="block text-[10px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-[0.25em] px-2">{{ __('forms.new_password') }}</label>
        <input id="password" type="password" name="password" required autofocus dir="ltr" placeholder="{{ __('forms.password_placeholder') }}"
            class="block w-full px-6 py-5 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl text-slate-900 dark:text-white font-black text-sm outline-none focus:ring-8 focus:ring-primary/5 focus:border-primary transition-all text-start shadow-sm" />
        @if($errors->has('password'))
            <p class="mt-2 font-black text-red-500 text-[10px] uppercase tracking-tighter px-2">
                @if(str_contains($errors->first('password'), '8'))
                    {{ __('forms.password_min_length') }}
                @elseif(str_contains($errors->first('password'), 'confirmation') || str_contains($errors->first('password'), 'تطابق'))
                    {{ __('forms.password_mismatch') }}
                @else
    ...
    {{-- تأكيد كلمة المرور --}}
    <div class="space-y-4 text-start">
        <label for="password_confirmation" class="block text-[10px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-[0.25em] px-2">{{ __('forms.confirm_password') }}</label>
        <input id="password_confirmation" type="password" name="password_confirmation" required dir="ltr" placeholder="{{ __('forms.password_placeholder') }}"
            class="block w-full px-6 py-5 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl text-slate-900 dark:text-white font-black text-sm outline-none focus:ring-8 focus:ring-primary/5 focus:border-primary transition-all text-start shadow-sm" />
    </div>
    ...
    <div class="pt-4">
        <button type="submit" id="submit-btn" class="w-full flex items-center justify-center bg-primary text-white py-4 rounded-2xl font-black text-lg shadow-[0_10px_30px_rgba(244,80,24,0.3)] hover:scale-[1.02] active:scale-[0.98] transition-all duration-300">
            <span id="btn-text">{{ __('forms.update_password_btn') }}</span>
...
<div class="min-h-[100vh] lg:min-h-0 flex flex-col items-center justify-center w-full py-8 text-center">
    <div class="fade-in">
        <h2 class="text-4xl lg:text-5xl font-semibold tracking-tight text-slate-800 dark:text-zinc-100 leading-tight mb-8">
            {{ __('forms.stronger_password_means') }}
            <span class="block mt-4 pb-4 font-[900] glow-text">{{ __('forms.unshakable_security') }}</span>
        </h2>
    </div>
